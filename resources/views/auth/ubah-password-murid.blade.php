...
<div class="min-h-screen flex items-center justify-center px-4">
    <div class="w-full max-w-sm">
        ...
        <div class="text-center mb-6">
            <h1 class="font-fredoka text-2xl font-bold text-emerald-700">Ubah Password</h1>
            <p class="text-slate-500 mt-1">Ganti password akunmu</p>
        </div>

        <div class="bg-white rounded-3xl shadow-xl p-8">

            @if (session('sukses'))
                <div class="mb-4 rounded-2xl bg-emerald-50 text-emerald-700 text-sm px-4 py-3">{{ session('sukses') }}</div>
            @endif

            @if ($errors->any())
                <div class="mb-4 rounded-2xl bg-rose-50 text-rose-600 text-sm px-4 py-3">{{ $errors->first() }}</div>
            @endif
            ...
            <form method="POST" action="{{ route('password.update') }}">
                ...
                <button type="submit"
                    class="w-full rounded-2xl bg-emerald-500 hover:bg-emerald-600 text-white font-fredoka font-semibold text-lg py-3 shadow-lg transition mt-2">
                    Simpan
                </button>
            </form>
        </div>
        ...
        <p class="text-center mt-6">
            <a href="{{ route('home') }}" class="text-sm font-semibold text-emerald-600 hover:underline">← Kembali</a>
        </p>
